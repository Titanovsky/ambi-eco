AMB.Ambition.String = AMB.Ambition.String or {}
setmetatable( AMB.Ambition.String, { __index = string } )
...
function string.ToSafe( sString )
    if not KEYS then KEYS = AMB.Ambition.Global.Keys end
...
function string.Random( nChars )
    ...
    nChars = nChars or 1
    if ( nChars > 256 ) or ( nChars <= 0 ) then A.Error( 'Ambition.String', 'Random | nChars is very long or small ( 0 > nChars > 256)' ) return '' end
...
function string:MaxCharacters( nAmount, bWithEllipses )
    ...
    if not isnumber( nAmount ) then A.Error( 'Ambition.String', 'MaxCharacters | nAmount is not selected!' ) return self end
...
function string.FindObsceneWords( sStr )
	if not sStr or not isstring( sStr ) then A.Error( 'AMB.Ambition.String', 'RemoveObsceneWords | sStr is not selected!' ) return '' end

	return AMB.Ambition.Global.Words.OBSCENE[ sStr ]
end
