...
local C, Amb, UT, Draw = AMB.Ambition.Global.Colors, AMB.Ambition, AMB.Ambition.Utility, AMB.UI.Draw
...
function AMB.UI.GUI.DrawScrollBar( vguiScrollPanel, fPaint )
    if not ValidPanel( vguiScrollPanel ) then Amb.Error( 'UI.GUI', 'DrawScrollBar: selected unvalid vguiScrollPanel' ) return end
...
function AMB.UI.GUI.DrawScrollBarGrip( vguiScrollPanel, fPaint )
    if not ValidPanel( vguiScrollPanel ) then Amb.Error( 'UI.GUI', 'DrawScrollBarGrip: selected unvalid vguiScrollPanel' ) return end
    local sbar = vguiScrollPanel:GetVBar()
...
function AMB.UI.GUI.DrawScrollBarButtonUp( vguiScrollPanel, fPaint )
    if not ValidPanel( vguiScrollPanel ) then Amb.Error( 'UI.GUI', 'DrawScrollBarButtonUp: selected unvalid vguiScrollPanel' ) return end
    local sbar = vguiScrollPanel:GetVBar()
...
function AMB.UI.GUI.DrawScrollBarButtonDown( vguiScrollPanel, fPaint )
    if not ValidPanel( vguiScrollPanel ) then Amb.Error( 'UI.GUI', 'DrawScrollBarButtonDown: selected unvalid vguiScrollPanel' ) return end
    local sbar = vguiScrollPanel:GetVBar()
...
function AMB.UI.GUI.DrawCheckBox( vguiParent, xPos, yPos, sFont, cColorText, sText, bValue, sConVar, bSizeToContent )
    ...
    frame:SetPos( xPos or 0, yPos or 0 )
    frame:SetFont( sFont or 'Default' )
    frame:SetTextColor( cColorText or C.ABS_WHOTE )
    frame:SetText( sText or '' )
...
function AMB.UI.GUI.DrawTextEntry( vguiParent, wSize, hSize, xPos, yPos, sFont, cColorText, sValue, cColorTextPlaceholder, sPlaceholder, bMultiline, bOnlyNumber )
    ...
    frame:SetPos( xPos or 0, yPos or 0 )
    frame:SetFont( sFont or 'Default' )
    frame:SetTextColor( cColorText or C.AMB_BLACK )
    frame:SetValue( sValue or '' )
    frame:SetPlaceholderColor( cColorTextPlaceholder or C.AMB_GRAY )
...
function AMB.UI.GUI.DrawModel( vguiParent, wSize, hSize, xPos, yPos, sModelPath, nSkin, sBodygroup )
...
function AMB.UI.GUI.DrawModel3D( vguiParent, wSize, hSize, xPos, yPos, sModelPath )
...
-- Options --------------------------------------------------------------------------------------
function AMB.UI.GUI.OnCursor( vguiPanel, fOnCursorEntered, fOnCursorExited )
    if not ValidPanel( vguiPanel ) then Amb.Error( 'UI.GUI', 'OnCursor: selected unvalid vguiPanel' ) return end
...
function AMB.UI.GUI.OnClick( vguiPanel, fLeftClick, fRightClick, fMiddleClick )
    if not ValidPanel( vguiPanel ) then Amb.Error( 'UI.GUI', 'OnClick: selected unvalid vguiPanel' ) return end
...
function AMB.UI.GUI.OnRemove( vguiPanel, fOnRemove )
    if not ValidPanel( vguiPanel ) then Amb.Error( 'UI.GUI', 'OnRemove: selected unvalid vguiPanel' ) return end
...
-- Compability ---------------------------------------------------------------------------
AMB.UI.GUI.OnCursorButton = AMB.UI.GUI.OnCursor